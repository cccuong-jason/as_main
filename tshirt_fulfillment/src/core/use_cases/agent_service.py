...
from core.domain.agent import AgentSession, AgentStatus, AgentRole
...
class AgentService:
    ...
    def _create_agent_session(
        ...
        try:
            ...
            tool_handlers = [tool for tool in tools]
    ...
    def create_admin_session(self, command_id: str) -> AgentServiceResult:
        ...
        try:
            ...
            return self._create_agent_session(
                session=session,
                tools=tools,
                system_prompt="You are an AI admin agent."
            )
        except Exception as e:
            return AgentServiceResult(
                success=False,
    ...
    def execute_session(self, session: AgentSession) -> AgentServiceResult:
        ...
        try:
            ...
            if session.role == AgentRole.CUSTOMER:
                # Get order
                order_result = self.order_processor.get_order(
                    session.order_id
                )
                if not order_result.success:
                    raise ValueError(
                        f"Order not found: {session.order_id}"
                    )
                # Execute agent
